...
const validateLinks = (arrayLinks) => {
    // Cada enlance tiene su propia promesa
    return Promise.all( // Devuelve una promesa que se resuelve cuando todas las promesas en el iterable se han resuelto
        arrayLinks.map((link) => { // Itera sobre cada elemento del arrayLinks
            // Fetch realiza la solicitud HTTP 
            return fetch(link.href)
                .then((response) => {
                    link.status = response.status
                    link.success = response.ok ? 'ok' : 'fail'
                    return link
                })
                .catch(() => {
                    link.status = 500
